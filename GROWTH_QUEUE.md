...
## Done
- [x] P1 Candidate A / EXP-112: 将 publish-daily 生成阶段前置为质量闸门（EN 不再发布原始中文 cron summary + 自动生成双语结构化正文 + CI 接入发布模板检查），消费 EXP-111 的发布前置假设 | ICE 9x8x8=576 — commit `(this commit)`
  - Hypothesis: 若在 `publish-daily.sh` 生成阶段直接阻断 EN 原始中文正文、缺失 H1/结论与构建 warning，而不是等发布后回补，可减少最新日报进入索引窗口前的语言错位、摘要薄弱和三标签结论缺失风险。
  - Metrics: `pnpm check:publish-daily-template`、`pnpm check:latest-daily-en-language`、`pnpm check:daily-template`、`pnpm check:daily-heading-date`、`pnpm check:daily-cta`、`pnpm check:rolling-daily-body`、`pnpm check:duplicate-slug-id`、`pnpm check:build-duplicate-id-warning` 与 `pnpm build` 全部通过。
  - Acceptance: 1) `publish-daily.sh` 新增 `EN_BODY`/`ZH_BODY` 生成，EN 文件不再写入原始 `${SUMMARY}`；2) ZH 自动补 H1 与三标签今日结论；3) publish 流程提交前运行核心日报质量闸门与严格 duplicate-id warning 闸门；4) 新增 `check:publish-daily-template` 并接入 content-check CI；5) 本地检查与构建全部通过。
...
- [x] P1 Candidate A / EXP-107: 修复 2026-05-09 双语日报发布窗口质量回归（EN 正文英文化 + 双语 description 可检索化 + 双语结论三标签补全 + 截断正文清理），优先消费最近24小时内容建设新增日报质量假设 | ICE 9x8x8=576 — commit `(this commit)`
  ...
  - Acceptance: 1) EN `openclaw-daily-2026-05-09.md` H1 与正文改为英文；2) EN/ZH description 升级为覆盖 Trusted Contact、MRC 训练网络、NVIDIA/DOE AI 超算、中国 AI 应用整治与 6G 试验频谱的可检索摘要；3) EN/ZH 结论补齐为三标签结构；4) 清理双语实战案例截断省略号；5) 保持 3 条强相关 CTA 内链；6) 本地检查与构建全部通过。
...
- [x] P1 Candidate A / EXP-104: 将最新日报质量闸门从发布窗口扩展到滚动 7 篇（EN 语言一致性 3→7 + 双语正文完整性 4→7），并回补 2026-04-30 EN/ZH 日报语言错位、结论结构与截断正文，消费 EXP-102/EXP-100 的滚动窗口扩展假设 | ICE 9x8x8=576 — commit `(this commit)`
  ...
  - Acceptance: 1) `scripts/check-latest-daily-en-language.sh` 默认窗口从 3 扩展到 7；2) `scripts/check-rolling-daily-body-completeness.sh` 默认窗口从 4 扩展到 7；3) EN `openclaw-daily-2026-04-30.md` 改为英文 H1、英文正文、英文 Takeaways；4) ZH `2026-04-30` 今日结论升级为三标签结构并清理截断省略号；5) 本地检查与构建全部通过。
- [x] P1 Candidate A / EXP-103: 修复 2026-05-07 双语日报发布窗口质量回归（EN 正文英文化 + 双语 description 可检索化 + 双语结论补全 + 截断来源清理），优先消费最近24小时内容建设新增日报质量假设 | ICE 9x8x8=576 — commit `(this commit)`
  - Hypothesis: 对最新发布日报中 EN 页面中文正文/缺失英文 H1、ZH 截断型 description、双语结论缺失与来源截断进行发布窗口内修复，可在索引前恢复语言一致性、摘要可检索性、页面完整性与站内导流质量。
  - Metrics: `pnpm check:latest-daily-en-language` 通过；`pnpm check:daily-template` 通过；`pnpm check:daily-heading-date` 通过；`pnpm check:daily-cta` 通过；`pnpm check:rolling-daily-body` 通过；`pnpm check:duplicate-slug-id` 通过；`pnpm build` 通过。
  - Acceptance: 1) EN `openclaw-daily-2026-05-07.md` H1 与正文改为英文；2) EN/ZH description 升级为覆盖 GPT-5.5 Instant、B2B Signals、MRC、GitHub agent 验证与具身智能安全的可检索摘要；3) EN/ZH 结论补齐为三标签结构；4) 清理新华网来源截断省略号；5) 保持 3 条强相关 CTA 内链；6) 本地检查与构建全部通过。
...
- [x] P1 Candidate A / EXP-097: 修复 2026-05-04 双语日报发布窗口质量回归（EN 正文英文化 + ZH description 可检索化 + 双语结论补全 + 截断来源 URL 修复），优先消费最近24小时内容建设新增日报质量假设 | ICE 9x8x8=576 — commit `6e05091`
  - Hypothesis: 对最新发布日报中 EN 页面中文正文/中文 H1、ZH 截断型 description、缺失结论段与截断来源 URL 进行发布窗口内修复，可在索引前恢复语言一致性、摘要可检索性、来源可信度与站内导流质量。
  - Metrics: `pnpm check:latest-daily-en-language` 通过；`pnpm check:daily-template` 通过；`pnpm check:daily-heading-date` 通过；`pnpm check:daily-cta` 通过；`pnpm build` 通过；`/en|zh/blog/openclaw-daily-2026-05-04/` 无 EN/ZH 语言错位、无截断 description、无结论缺失。
  - Acceptance: 1) EN `openclaw-daily-2026-05-04.md` H1 与正文改为英文；2) ZH description 从截断正文升级为可检索摘要；3) EN/ZH 今日结论补全；4) 修复 Anthropic 来源截断 URL；5) 保持 3 条强相关 CTA 内链；6) 本地检查与构建全部通过。
...
- [x] P1 Candidate A / EXP-092: 回补 2026-04-30 双语日报 description（修复 EN 通用摘要与 ZH 截断异常，升级为可检索摘要并保持强相关 CTA 内链），优先消费最近24小时内容建设延续假设 | ICE 9x8x8=576 — commit `(this commit)`
  ...
  - Acceptance: 1) 更新 EN/ZH `openclaw-daily-2026-04-30.md` description 为可检索摘要；2) 保持 EN/ZH 页面 3 条强相关 CTA 内链不回退；3) 本地检查与构建全部通过。
- [x] P1 Candidate A / EXP-091: 回补 2026-04-22 双语日报 description（将异常/截断文案升级为可检索摘要，优先消费最近24小时内容建设新增实验假设），保持 CTA 强相关内链并完成 build 闭环 | ICE 9x8x8=576 — commit `(this commit)`
  - Hypothesis: 对最近24小时新发布且出现 description 异常（EN 通用摘要、ZH 截断/噪音摘要）的双语日报页执行可检索化回补，可提升主题检索匹配、摘要点击意图一致性与核心指南导流质量。
  - Metrics: `pnpm check:daily-template` 通过；`pnpm check:daily-heading-date` 通过；`pnpm check:daily-cta` 通过；`pnpm build` 通过；`/en|zh/blog/openclaw-daily-2026-04-22/` description 不再为通用/截断文案且覆盖当日核心主题。
  - Acceptance: 1) 更新 EN/ZH `openclaw-daily-2026-04-22.md` description 为可检索摘要；2) 保持 EN/ZH 页面 3 条强相关 CTA 内链不回退；3) 本地检查与构建全部通过。
