export const fixtureDate = '2026-08-15';
...
发生了什么：在 GTC Taipei，NVIDIA 发布 Cosmos 3，称其为开放的物理 AI 世界基础模型，可原生理解与生成文本、图像、视频、环境音与动作，用于机器人、自动驾驶与视觉 AI 的合成数据、仿真与策略训练，并成立 Cosmos 联盟。  
为什么重要：开源世界基础模型是机器人、自动驾驶与视觉 AI 的底层引擎，物理 AI 训练与评估正在从单一前端模型转向可复用的仿真与合成数据基础设施。  
可能影响：中小机器人、智驾与视觉 AI 团队能以更低成本接入顶尖世界模型，但同时更依赖 NVIDIA 的算力与软件栈。  
状态：已确认，L1 官方直抓。

2. 月之暗面全量开源 Kimi K3，中国开源大模型全球下载量破 100 亿次  
发生了什么：月之暗面全量开源 Kimi K3，号称全球参数规模最大的开源模型，单模型 2.8 万亿参数、100 万 token 上下文；新华网报道中国开源模型全球累计下载量突破 100 亿次，海外惊呼新一轮冲击。  
为什么重要：开源正在重塑 AI 竞争格局，中国开源模型从追赶者变成生态规则参与者，开发者与企业的默认选项正在变化。  
可能影响：全球大模型下载中来自中国模型的比例持续上升，企业与开发者会更频繁地比较中文推理、工具调用、长上下文编码与本地部署能力。  
状态：已确认，L2 权威媒体直抓。

3. NVIDIA CEO Jensen Huang 登顶 Glassdoor 2026 全球最佳 CEO  
发生了什么：Glassdoor 发布 2026 年榜单，NVIDIA CEO Jensen Huang 以 99% 员工认可度排名第一，科技行业有 9 位 CEO 入榜，是入榜最多的行业。  
...
为什么重要：比赛规则会演进成技术验收标准，人形机器人正从演示加速走向力量、平衡、操作与协调的可度量能力分级。  
可能影响：人形机器人团队应关注赛事表现能否转化为可验证的能力分级、可采购规格以及面向工业、家政、消防等真实场景的安全证据。  
状态：已确认，L2 权威媒体直抓。

5. NVIDIA 把物理 AI 工具开源给 Agent：智能体开始编排造机器人  
发生了什么：NVIDIA 发布一批开源的 Physical AI Agent Skills，让编码 Agent 直接调用 Omniverse 等库完成数据生成、仿真、训练，相关企业靠合成数据把训练部署时间缩短。  
...
1. 制造业合成数据补缺陷样本  
用合成缺陷数据训练视觉检测模型，是制造业 AI 的普遍破局点：缺缺陷样本时用合成数据补样本，可把训练与部署时间明显缩短。  
实际价值：  
- 合成缺陷样本降低数据采集成本；  
...
export const expectedSignals = [
  ...
  { title: '月之暗面全量开源 Kimi K3，中国开源大模型全球下载量破 100 亿次', sourceProjectionRuleMatches: ['moonshot-kimi-k3-long-context-open-model-2026'], enLabel: 'China / Kimi K3 / long-context open model', zhEvidence: '来源条目 2：月之暗面全量开源 Kimi K3，中国开源大模型全球下载量破 100 亿次', requiredTokens: ['Kimi K3', '2.8 万亿参数', '100 万 token', '100 亿次'] },
...
export const requiredZhOutputs = [
  ...
  '月之暗面全量开源 Kimi K3，中国开源大模型全球下载量破 100 亿次',
...
export const parserGuardrails = {
  ...
  story2RequiredDetailTokens: ['Kimi K3', '2.8 万亿参数', '100 万 token', '100 亿次'],
